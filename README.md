...
- **Telegram-бот «Барс»** — Python + LangGraph + Gemini. RAG-агент, который находит
  мероприятия в том же каталоге, доводит до регистрации, составляет план подготовки
  и напоминает о дедлайнах. Подробности — в `bot/README.md`.

```
frontend/   React SPA (Vite)
backend/    NestJS REST API
bot/        Telegram-агент «Барс» (Python, LangGraph, Gemini)
...
cp .env.example .env    # ключ Gemini, токен бота, DATABASE_URL, BOT_API_SECRET
python -m bars.main --polling
```

Бот — отдельный сервис на Railway (root directory `bot`). Он ходит за данными
**только через этот же API**: публичные `/api/events*` плюс приватные `/api/bot/*`,
защищённые общим секретом `BOT_API_SECRET` (заголовок `X-Bot-Secret`). Напрямую в
Postgres он пишет только свои таблицы `bot_*`.

Привязка Telegram к аккаунту — необязательная: без неё Барс тоже подбирает
мероприятия и составляет планы, а с ней знает возраст из профиля и умеет добавлять
в избранное. Привязка всегда начинается на сайте: **Профиль → Привязать Telegram**
выдаёт одноразовую ссылку `t.me/<bot>?start=<token>`, которую бот гасит при переходе.
...
- Push/email-уведомления (переключатель `notif_opt_in` в профиле уже есть) — в
  Telegram напоминания о дедлайнах уже работают через бота.
